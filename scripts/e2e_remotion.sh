...
PROJECT=$(curl -s -c "$COOKIE_JAR" -b "$COOKIE_JAR" -X POST "$API/projects/" \
  ...
  -d '{"title": "E2E Remotion Test", "source_url": "https://example.com"}')
echo "$PROJECT" | jq .
PROJECT_ID=$(echo "$PROJECT" | jq -r '.id')

echo "=== 3. Trigger agent-generate with engine=remotion ==="
JOB=$(curl -s -c "$COOKIE_JAR" -b "$COOKIE_JAR" -X POST "$API/projects/$PROJECT_ID/renders/agent-generate" \
  -H "Content-Type: application/json" \
  -d '{"prompt":"make a 2 second demo clip","engine":"remotion"}')
echo "$JOB" | jq .
...
if echo "$FILE_TYPE" | grep -qi "mp4\|iso media\|apple quicktime"; then
  echo "PASS: real MP4 generated via worker e2e"
else
